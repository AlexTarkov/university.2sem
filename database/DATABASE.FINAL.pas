...
procedure writeBase(var a:base);
	...
	for i:=1 to a[0].stage do
		begin
		write( '| ' , a[i].fio+StringOfChar( ' ' , 30-length(a[i].fio) ));
		write('| ' , IntToStr(a[i].stage), StringOfChar(' ', 8-length(IntToStr(a[i].stage)) ) );
		writeln('| ' , IntToStr(a[i].price), StringOfChar(' ', 8-length(IntToStr(a[i].price)) ) );
		end;
	writeln('--------------------------------------------------------------');
	end;
